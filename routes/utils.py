...
def dfs_paths(graph, start, goal):
    ...
    в другой, Вариант посещения одного и того же города более
...
def get_routes(request, form) -> dict:
    ...
    if cities:
        _cities = [city.id for city in cities]  # Генераторное выражение
        right_ways = []
        for route in all_ways:
            if all(city in route for city in _cities): # Генераторное выражение
    ...
    for route in right_ways:
        ...
        if total_time <= travelling_time:
            ...
            # то добавляет маршрут в общий список.
            routes.append(tmp)
    if not routes:
        # Если список пуст, то нет никаких маршрутов,
