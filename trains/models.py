...
class Train(models.Model):
    ...
    from_city = models.ForeignKey(
        City, on_delete=models.CASCADE, related_name='from_city_set', verbose_name='Из какого города'
    )
    to_city = models.ForeignKey(
        "cities.City", on_delete=models.CASCADE, related_name='to_city_set', verbose_name='В какой город'
    )
    ...
    class Meta:
        ...
        ordering = ['travel_time']  # сортировать по
    ...
    def clean(self):
        ...
        ).exclude(pk=self.pk)  # Исключение. Train == self.__class__
        if qs.exists():
...
class TrainTest(models.Model):
    name = models.CharField(max_length=50, unique=True, verbose_name='Номер поезда')
    from_city = models.ForeignKey(
        City, on_delete=models.CASCADE, related_name='from_city',
